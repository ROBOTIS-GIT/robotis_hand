...
#include "robotis_hand_playground/tactile_hold_controller.hpp"
  ...
  for (int i = 0; i < fingers_num; ++i) {
    ...
    if (!finger.contact_detected) {
      if (i == 0) {
        ...
        for (int j = 2; j <= 3; ++j) {
          finger.current_joint_targets[j] += direction * 2 * param.close_step * weights[j];
          finger.current_joint_targets[j] =
            clamp(finger.current_joint_targets[j], finger.joint_min[j], finger.joint_max[j]);
        }
      } else {
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<robotis_hand_playground_hold::TactileHoldController>());
  rclcpp::shutdown();
  return 0;
}
